                                ...
                                (item.category_id === 3) ? (
                                    <a href={item.id} className="grid-item" key={item.id}>
                                        <section id="grid-content">
                                            <img key={index} src={isActive1.activeObject === index ? (item.b_image) : (item.a_image)} alt={item.name + ' image'}  onMouseEnter={() => { toggleActive(index) }} onMouseLeave={() => { setIsActive1({ ...isActive1, activeObject: null }) }} />
                                            <p>{item.name}</p>
