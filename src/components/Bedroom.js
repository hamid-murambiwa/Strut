                ...
                <Slide bottom>
                  <Link to={`/shop/${item.id}`} className="grid-item" key={item.id}>
                    <section id="grid-content">
                      <img key={item.id} src={isActive1.activeObject === index ? (item.b_image) : (item.a_image)} alt={`${item.name}`} onMouseEnter={() => { toggleActive(index); }} onMouseLeave={() => { setIsActive1({ ...isActive1, activeObject: null }); }} onTouchStart={() => { toggleActive(index); }} onTouchEnd={() => { setIsActive1({ ...isActive1, activeObject: null }); }} />
                      <p>{item.name}</p>
                      <p className="price">
                        R
                        {sort(item.price)}
